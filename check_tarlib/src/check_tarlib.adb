...
procedure Check_Tarlib is
   ...
   begin
      ...
      --
      --  This asked instead that each name appear somewhere in the README or
      --  the docs, which a name can do while nothing about it is explained.
      --  Require_Public_GNATdoc_Tags is project_tools' own rule and asks for a
      --  description with @param and @return. Tags follow the declaration in
      --  this crate, so Tags_Before is False; the default would match each
      --  declaration against its neighbour's tags and pass for the wrong
      --  reason.
      declare
      ...
      begin
         ...
         while Ada.Directories.More_Entries (Search) loop
            ...
            --  Tarlib.Internal is not interface and is not asked.
            if Project_Tools.Text.Index
                 (Ada.Directories.Simple_Name (Item), "internal") = 0
            then
               begin
